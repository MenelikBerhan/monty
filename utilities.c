...
void assign_f(ins_t *ins)
{
	...
	op = ins->opcode;
	if (!op || op[0] == '#' || !strncmp(op, "nop", 3))
		ins->f = nop;
	else if (!strncmp(op, "push", 4))
		ins->f = push;
	else if (!strncmp(op, "pop", 3))
		ins->f = pop;
	else if (!strncmp(op, "pall", 4))
		ins->f = pall;
	else if (!strncmp(op, "pint", 4))
		ins->f = pint;
		...
		ins->f = mod;
	else if (!strncmp(op, "pchar", 4))
		ins->f = pchar;
	else if (!strncmp(op, "pstr", 4))
		ins->f = pstr;
	else if (!strncmp(op, "rotl", 4))
		ins->f = rotl;
	else if (!strncmp(op, "rotr", 4))
		ins->f = rotr;
	else if (!strncmp(op, "stack", 5))
		ins->f = _stack;
	else if (!strncmp(op, "queue", 5))
		ins->f = queue;
	else
		ins->f = NULL;
}
